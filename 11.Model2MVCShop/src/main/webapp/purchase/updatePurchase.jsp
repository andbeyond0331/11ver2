...
<%
	...
	System.out.println("updatePurchase의 purchaseVO: "+purchaseVO);
	
	User userVO = (User)session.getAttribute("user");
	
	String trimPaymentOption = purchaseVO.getPaymentOption().replace(" ", "");
%>	//////////// --%>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0"	style="margin-top: 13px;">
	...
	<tr>
		...
		<td bgcolor="D6D6D6" width="1"></td>
		<td class="ct_write01">
		<c:if test="${!empty purchase.paymentOption && purchase.paymentOption eq '1' }">
			현금결제
		</c:if>
		<c:if test="${!empty purchase.paymentOption && purchase.paymentOption eq '2' }">
			카드결제
		</c:if>
		<%-- 
		<%if(trimPaymentOption!=null){ %>
      		<%if(trimPaymentOption.equals("1")){ %>
       	  		현금결제
      		<%}else if(trimPaymentOption.equals("2")){ %>
      			카드결제
      		<%} %>
     	 <%} %>--%>
		</td>
